...
PROJECT_ID = "cymo-hogent-2025-operator"  # Replace with your actual GCP project ID
SECRET_ID = "confluent-api-credentials"  # Fixed name for the secret in GSM
...
@kopf.on.create('v1', 'secrets', labels={'app': 'confluent'})
def sync_confluent_credentials(spec, meta, body, namespace, logger, **kwargs):
    if meta['name'] != CLOUD_KEY_SECRET or namespace != NAMESPACE_ARGOCD:
        return

    if gsm is None:
        logger.error("Google Secret Manager client not initialized. Cannot sync credentials.")
        return

    logger.info("🎯 New Confluent credentials detected, pushing to Google Secret Manager...")

    try:
        # Use existing get_confluent_credentials function
        api_key, api_secret = get_confluent_credentials(CLOUD_KEY_SECRET, namespace)
        combined = f"{api_key}:{api_secret}".encode("utf-8")
        ...
        try:
            ...
            logger.info(f"✅ Google Secret Manager secret created: {SECRET_ID}")
        except Exception as e:
            if "AlreadyExists" in str(e):
                logger.info(f"ℹ️ Google Secret Manager secret {SECRET_ID} already exists.")
            else:
                raise

        # Add new version
        response = gsm.add_secret_version(
            request={
                "parent": secret_path,
        ...
        logger.info(f"🔐 Secret version added to Google Secret Manager: {response.name}")
    except Exception as e:
        logger.error(f"Failed to sync credentials to Google Secret Manager: {e}")
        raise

@kopf.on.update('v1', 'secrets', labels={'app': 'confluent'})
def update_confluent_credentials(spec, meta, body, namespace, logger, **kwargs):
    if meta['name'] != CLOUD_KEY_SECRET or namespace != NAMESPACE_ARGOCD:
        return
    ...
    logger.info("🔄 Confluent credentials updated, syncing to Google Secret Manager...")
    ...
    try:
        ...
        logger.info(f"🔐 Updated secret version added to Google Secret Manager: {response.name}")
    except Exception as e:
        logger.error(f"Failed to sync updated credentials to Google Secret Manager: {e}")
        raise
